...
#include "dil/can_types.h"                                       // CANHeader, CAN_ID_SYSTEM_STATE
 ...
 *   - 29-bit id  = CANHeader { senderID, targetID, messageID = CAN_ID_SYSTEM_STATE,
...
inline void packSystemState(const SystemState& record, uint8_t sender, uint8_t target,
                            uint8_t seq, std::span<CanFrame, SYSTEM_STATE_FRAGMENTS> out)
{
    ...
    header.frame.messageID   = CAN_ID_SYSTEM_STATE;
...
/* Reassembles SystemState records from inbound CAN_ID_SYSTEM_STATE fragments, one
...
public:
    std::optional<SystemState> accept(const CanFrame& frame)
    {
        ...
        if (header.frame.messageID != CAN_ID_SYSTEM_STATE || frame.length < 1) {
            return std::nullopt;
        }
